  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-xs transition-colors dark:border-slate-800 dark:bg-slate-900">
      ...
      <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {actions.map(({ href, title, desc, icon: Icon, color, badge, badgeVariant }) => {
          const colorStyles = {
          ...
          return (
            <Link
              key={href}
              href={href}
              className="group flex flex-col justify-between rounded-xl border border-slate-200 bg-slate-50/60 p-4 transition hover:border-blue-500 hover:bg-blue-50/30 dark:border-slate-800 dark:bg-slate-800/40 dark:hover:border-blue-500/70 dark:hover:bg-slate-800/80 focus:outline-hidden focus:ring-2 focus:ring-blue-500/20"
            >
              <div className="flex items-start justify-between gap-3">
                <div className="flex items-center gap-3">
                  <div className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-lg ${colorStyles}`}>
              ...
              {badge && (
                <div className="mt-3 flex items-center justify-start">
                  <span className="inline-flex rounded-md bg-amber-100 px-2 py-0.5 text-[10px] font-extrabold uppercase tracking-wide text-amber-800 dark:bg-amber-950/60 dark:text-amber-300">
                    {badge}
                  </span>
                </div>
              )}
